...
if (isset($_GET['table']) && $_GET['table'] == 'staffs') {
    ...
    foreach ($res as $row) {
        ...
        $tempRow['branch'] = $row['branch'];
        $sql = "SELECT COUNT(id) AS total FROM `staff_transactions` WHERE DATE(datetime) = '$currentdate' AND amount = 50 AND staff_id = $staff_id GROUP BY user_id";
        $db->sql($sql);
        $res = $db->getResult();
        $direct_join = $res[0]['total'];
        $sql = "SELECT COUNT(id) AS total FROM `staff_transactions` WHERE DATE(datetime) = '$currentdate' AND amount = 7.50 AND staff_id = $staff_id GROUP BY user_id";
        $db->sql($sql);
        $res = $db->getResult();
