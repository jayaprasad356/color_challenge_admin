...
if (isset($_GET['table']) && $_GET['table'] == 'users') {
    ...
    foreach ($res as $row) {
        ...
        $tempRow['d_referred_by'] = $row['d_referred_by'];
...
if (isset($_GET['table']) && $_GET['table'] == 'scratch_cards') {
    ...
    if (isset($_GET['order']))
        ...
        $sql = "SELECT l.id AS id, l.*, u.name FROM `scratch_cards` l " . $join . " ORDER BY $sort $order LIMIT $offset, $limit";
        $db->sql($sql);
        $res = $db->getResult();
    ...
    foreach ($res as $row) {
        ...
        $operate = '<a href="edit-scratch_cards.php?id=' . $row['id'] . '"><i class="fa fa-edit"></i>Edit</a>';
        $operate .= ' <a class="text text-danger" href="delete-scratch_cards.php?id=' . $row['id'] . '"><i class="fa fa-trash"></i>Delete</a>';
       $tempRow['id'] = $row['id'];
       $tempRow['name'] = $row['name'];
       $tempRow['amount'] = $row['amount'];
       ...
       $tempRow['status'] ="<p class='text text-success'>Approved</p>";
   elseif($row['status']==0)
       $tempRow['status']="<p class='text text-primary'>Not-Approved</p>";
   else
...
//plan_details
if (isset($_GET['table']) && $_GET['table'] == 'plan_details') {
    ...
    $sql = "SELECT COUNT(`id`) as total FROM `plan_details` ";
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row)
        $total = $row['total'];
   
    $sql = "SELECT * FROM plan_details " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    $db->sql($sql);
    $res = $db->getResult();
    ...
    foreach ($res as $row) {
        ...
        $operate = ' <a href="edit-plan_details.php?id=' . $row['id'] . '"><i class="fa fa-edit"></i>Edit</a>';
        $operate .= ' <a class="text text-danger" href="delete-plan_details.php?id=' . $row['id'] . '"><i class="fa fa-trash"></i>Delete</a>';
        $tempRow['id'] = $row['id'];
        $tempRow['name'] = $row['name'];
...
/*if (isset($_GET['table']) && $_GET['table'] == 'user_plan') {
    ...
    if (isset($_GET['order']))
        ...
        if (isset($_GET['search']) && !empty($_GET['search'])) {
            $search = $db->escapeString($_GET['search']);
            $where .= " AND (u.id LIKE '%" . $search . "%' OR u.name LIKE '%" . $search . "%' OR p.name LIKE '%" . $search . "%')";
        }
        $join = "LEFT JOIN `users` u ON l.user_id = u.id LEFT JOIN `plan_details` p ON l.plan_id = p.id WHERE l.id IS NOT NULL " . $where;
        ...
        $sql = "SELECT l.id AS id, l.*, u.name AS user_name, p.name AS plan_name FROM `user_plan` l " . $join . " ORDER BY $sort $order LIMIT $offset, $limit";
        $db->sql($sql);
        $res = $db->getResult();
